...
async function analyzeWithTrainingData(
  ...
  try {
    ...
    if (exactMatch) {
      ...
      const jumperConnections = (exactMatch.jumper_connections as JumperConnection[]) || []
      ...
      return {
        ...
        reasons: [
          ...
          `Found ${jumperConnections.length} jumper connections`,
...
function enhanceWithTrainingData(analysis: any, similarConfigs: any[]) {
  try {
    ...
    const jumperPatterns: Record<string, number> = {}

    similarConfigs.forEach((config) => {
      const connections = config.user_verified_connections as WireConnection[]
      const jumpers = (config.jumper_connections as JumperConnection[]) || []
      ...
      // Track jumper patterns
      jumpers.forEach((jumper: JumperConnection) => {
        const jumperKey = `${jumper.fromTerminal}-${jumper.toTerminal}`
        jumperPatterns[jumperKey] = (jumperPatterns[jumperKey] || 0) + 1
      })
    })
    ...
    // Add common jumper patterns from training data
    const commonJumpers = Object.entries(jumperPatterns)
      .filter(([, count]) => count >= Math.max(1, similarConfigs.length * 0.3))
      .map(([jumperKey]) => {
        const [fromTerminal, toTerminal] = jumperKey.split("-")
    ...
    const enhancedReasons = [
      ...
      `Most common terminals in similar setups: ${Object.entries(terminalFrequency)
        ...
        .join(", ")}`,
      ...(commonJumpers.length > 0
        ? [`Common jumper patterns: ${commonJumpers.map((j) => `${j.fromTerminal}→${j.toTerminal}`).join(", ")}`]
        : []),
    ]
    ...
    return {
      ...
      jumperConnections: [...(analysis.jumperConnections || []), ...commonJumpers],
...
5. DETECT JUMPER WIRES - copper connections between terminals
...
COMMON JUMPER PATTERNS:
1. R to Rc jumper (most common)
2. R to Rh jumper
3. Rc to Rh jumper
      ...
      return {
        ...
        connectedWires:
          analysisData.wireConnections
            ?.filter((conn: WireConnection) => conn.hasWire)
            ?.map((conn: WireConnection) => conn.terminal) || [],
        jumperConnections: analysisData.jumperConnections || [],
